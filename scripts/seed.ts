/**
 * Seed script — populates the Payload SQLite database with the 6 casinos
 * from the original hardcoded lib/casinos.ts data.
 *
...
// ─── Original casino data ──────────────────────────────────────────────────

const casinosData = [
  {
    slug: 'bet99',
    name: 'Bet99',
    logoUrl: '/logos/bet99.svg',
    rating: 4.7,
    welcomeBonus: '100% up to $500',
    bonusCode: 'MAPLE99',
    affiliateUrl: 'https://bet99.com',
    province: 'Both' as const,
    featured: true,
    featuredRank: 1,
    pros: [
...
async function seed() {
  console.log('🌱 Starting Maple Casinos seed...')

  const payload = await getPayload({ config })

  for (const casino of casinosData) {
    const { pros, cons, paymentMethods, fullReview, ...rest } = casino

    // Check if casino already exists
    const existing = await payload.find({
      collection: 'casinos',
      where: { slug: { equals: casino.slug } },
      limit: 1,
    })

    if (existing.docs.length > 0) {
      console.log(`  ⏭️  Skipping ${casino.name} (already exists)`)
      continue
    }

    // Build Lexical richText structure from plain text
    const paragraphs = fullReview.split('\n\n').filter(Boolean)
  ...
  try {
    await payload.updateGlobal({
      ...
      data: {
        ...
        siteDescription: "Canada's #1 Casino Review Site — Ontario & Alberta",
    ...
    console.log('  ⚠️  Could not update site settings:', err)
  }
  ...
  console.log('  3. Create your admin account with email: admin@maplecasinos.ca')
  process.exit(0)
}
